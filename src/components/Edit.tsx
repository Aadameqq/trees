...
export function Edit({ initialAd, adId }: { initialAd: Ad; adId: string }) {
  const { classes } = useStyles();
  const [currentStep, setCurrentStep] = useState(0);

  const { mutate, isLoading } = useEditAd(adId);
          ...
          <Stepper.Completed>
            <Paper p="xl">
              <Text fw={500} fz={30} align={'center'}>
                Twoje ogłoszenie zostało pomyślnie zedytowane!
              </Text>
            </Paper>
          </Stepper.Completed>
        </Stepper>
        ...
        <Group position="right" mt={30}>
          ...
          {currentStep === 3 ? (
            ...
            <>
              ...
              <Link href={`/ads/${adId}`} className={classes.btn}>
